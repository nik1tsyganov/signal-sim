...
- `go-nogo` — daily paper checklist (`docs/decision/YYYY-MM-DD.json`). `TRADE` only when the book is off-target and feeds/research/clock/orders allow a submit. `--live` checks feed health. `rebalance --submit-paper` reads that artifact.
- `baseline-compare --fixtures` — walk-forward conviction vs equal-weight top-K on frozen fixture marks. Not alpha. Not fitted.
