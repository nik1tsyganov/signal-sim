...
This repository must not connect to live money. It contains no live trading engine and no credentials. With owner paper keys, it can read an Alpaca paper account and pull live intel counts. Local fills stay on the ledger. Remote Alpaca paper POSTs stay off unless `SIGNAL_SIM_ALPACA_PAPER_SUBMIT=1` and an explicit `paper-submit` / `--submit-paper` flag.
