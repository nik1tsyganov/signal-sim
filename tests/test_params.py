...
class ParamsManifestTests(unittest.TestCase):
    def test_module_constants_match_manifest(self):
        ...
        self.assertEqual(params.GO_NOGO_MIN_QUIVER_N, raw["go_nogo"]["min_quiver_n"])
        self.assertEqual(params.GO_NOGO_MIN_WORLDMONITOR_N, raw["go_nogo"]["min_worldmonitor_n"])
        self.assertAlmostEqual(params.GO_NOGO_SOFT_DD, raw["go_nogo"]["soft_dd"])
        self.assertAlmostEqual(params.GO_NOGO_HARD_DD, raw["go_nogo"]["hard_dd"])
        self.assertEqual(params.GO_NOGO_HARD_DD_BLOCKS, raw["go_nogo"]["hard_dd_blocks"])
        self.assertIn("not fitted", raw["go_nogo"]["note"].lower())
        go_nogo = params.go_nogo_params()
        self.assertAlmostEqual(go_nogo["trim_band"], raw["conviction"]["trim_band"])
        self.assertNotIn("go_nogo", params.frozen_operate_params())
        self.assertNotIn("min_quiver_n", params.frozen_operate_params())
