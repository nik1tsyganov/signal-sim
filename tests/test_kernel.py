...
class PaperOnlyCliTests(unittest.TestCase):
    ...
    def test_shadow_without_fixtures_flag_is_refused(self):
        output = io.StringIO()
        error = io.StringIO()
        with redirect_stdout(output), redirect_stderr(error):
    ...
    def test_rebalance_without_fixtures_flag_is_refused(self):
        output = io.StringIO()
        error = io.StringIO()
        with redirect_stdout(output), redirect_stderr(error):
            exit_code = cli.main(["rebalance"])
    ...
    def test_walkforward_without_fixtures_flag_is_refused(self):
        ...
        error = io.StringIO()
        with redirect_stdout(output), redirect_stderr(error):
            exit_code = cli.main(["walkforward"])
