...
`go-nogo` (alias `decision-check`) then reads that research book plus the latest paper-performance / telemetry snapshot and writes `docs/decision/YYYY-MM-DD.json`. Declared thresholds live in `fixtures/params.json` `go_nogo` (plus `conviction.trim_band` and `conviction.min_realize_loss_bps`). Verdict is `TRADE` | `HOLD` | `WAIT_OPEN` | `NO_GO`. `recommend_submit` is true only for `TRADE` (actionable off-target beyond the trim band, leftover **green** sells, or cash/gross off the book). Underwater leftovers / score-rotation trims are stamped `sell_blocked_reason=underwater_hold` on `deferred_exits` and do **not** force a submit. Open paper orders force `HOLD`. A closed clock is `WAIT_OPEN` (research is still OK). Missing today's book or dead feeds are `NO_GO`. `--live` also checks Quiver / World Monitor health and fail-closes if keys are missing. Optional `--md`. Not alpha.
...
Loads today's research artifact when present (otherwise computes the same book). Diffs that conviction book against paper positions: **buys and sells**. Rebalance aims to improve paper PnL, not just rotate names. Sell priority when several exits fire: **soft_stop ≥ horizon_exit (hard; still sell underwater) ≥ score_decay ≥ trim / drop_from_book (discretionary)**. Discretionary / rank exits do **not** sell when mark-to-market versus paper avg entry is red beyond `min_realize_loss_bps=5` (declared noise buffer, not fitted). Those names stay `hold_underwater` / `sell_blocked_reason=underwater_hold` (no POST). Soft-stops and other hard risk/horizon exits still fire when red. New buys prefer free cash / unused `max_gross_invest` room; if the book needs room, trim **winners** that are overweight versus target — do not dump underwater names that merely left top-k. Tickets stamp `sell_reason`. Qty prefers an observed paper IEX last trade or snapshot `latestTrade` when one exists. Soft-stop MTM uses that decision-time mark versus paper entry — no future bars. Fixture `$36`/`$40` QQQ/SPY marks are not used to size a live or `--submit-paper` ticket when a paper mark is available. Never invents a price.
