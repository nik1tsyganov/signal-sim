...
- **Cloud:** prefer saved environment `signal-sim-paper`. Runtime Secrets: `ALPACA_PAPER_API_KEY`, `ALPACA_PAPER_API_SECRET`, `QUIVER_API_KEY`, `WORLD_MONITOR_KEY`. Env: `ALPACA_PAPER_API_BASE_URL` (paper HTTPS origin), `SIGNAL_SIM_ALPACA_PAPER_SUBMIT` default `0`. `runtime-env` prints presence only. Never write secret values into the repo, PR, or logs.
