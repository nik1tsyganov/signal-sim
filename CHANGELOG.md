...
- **Note:** [research-conviction.md](docs/research-conviction.md). 2026-09-04 print-only A/B vs the equal-weight 10: enter GOOGL/HD/ABT/AMAT/UNH/AMZN; exit NFLX/CMCSA/CVX/DIS/SPY/XOM; NVDA highest frac (~0.18 artifact / 0.20 live with lags). No `--submit-paper` in that PR. The later authorized submit overwrote `docs/research/2026-09-04.json` with the conviction book. The A/B test now reads "before" from the frozen [2026-09-04-equal-weight.json](docs/research/2026-09-04-equal-weight.json) snapshot so later `research --live` writes do not break CI.
